...
interface DatasetManagerProps {
  ...
  onRemoveFile: () => void;
  onQuestionTextChange: (questionId: number, nextText: string) => void;
...
const GUIDE_STEPS = [
  {
    title: '기준 문서 업로드',
    description: '평가 기준이 되는 PDF 문서를 올립니다.',
  },
  {
    title: '질문 세트 생성',
    description: '문서 내용을 바탕으로 평가에 사용할 질문을 생성합니다.',
  },
  {
    title: '질문 검토/수정',
  ...
  {
    title: 'CSV 다운로드',
    description: '질문 파일을 내려받아 사용자의 RAG 시스템에서 답변을 생성합니다.',
  },
];
...
export default function DatasetManager({
  ...
  const workflowState = useMemo(() => {
    if (!uploadedFile) return '먼저 기준 PDF 문서를 업로드하세요.';
    if (!generatedSummary) return '문서가 준비되었습니다. 상단 버튼으로 질문을 생성하세요.';
    if (!hasDownloadedQuestionSet) return '질문을 검토한 뒤 CSV로 다운로드하세요.';
    return '질문 CSV 다운로드가 완료되었습니다. 이제 성능 평가 단계로 이동할 수 있습니다.';
  ...
  return (
    <div className="flex w-full flex-col gap-6">
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_8px_28px_rgba(15,23,42,0.04)]">
          ...
          <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
            <div className="max-w-4xl">
              ...
              <p className="mt-2 text-[15px] leading-7 text-slate-500">
                기준 문서를 업로드하고 질문을 생성한 뒤, 같은 화면에서 검토/수정하고 CSV로 내려받습니다.
              </p>
            </div>

            <div className="w-full max-w-lg rounded-xl border border-slate-200 bg-slate-50 px-5 py-4">
        ...
        <div className="grid gap-6 p-7 xl:grid-cols-[minmax(0,1fr)_400px]">
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <h3 className="text-xl font-semibold text-slate-950">기준 PDF 업로드</h3>
                <p className="mt-1 text-[15px] leading-7 text-slate-500">
                  평가의 기준이 될 문서를 선택하세요. 현재 UI는 PDF 기준으로 정리되어 있습니다.
                </p>
              </div>
              ...
              {uploadedFile ? (
                <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-slate-500">선택된 기준 문서</p>
                ...
                <div className="mx-auto max-w-2xl text-center">
                  ...
                  <p className="mt-2 text-[15px] leading-7 text-slate-500">
                    문서 업로드 후 상단의 질문 생성 버튼을 누르면 테스트셋 초안이 만들어집니다.
                  </p>
                  <button
                    type="button"
                    onClick={() => fileInputRef.current?.click()}
          ...
          <aside className="rounded-2xl border border-slate-200 bg-white p-6">
            <h3 className="text-base font-semibold text-slate-950">진행 안내</h3>
            <p className="mt-2 text-[15px] leading-7 text-slate-500">{workflowState}</p>
      ...
      {hasQuestions ? (
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_8px_28px_rgba(15,23,42,0.04)]">
          <div className="flex flex-col gap-4 border-b border-slate-100 px-7 py-6 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-4xl">
              ...
              <p className="mt-1 text-[15px] leading-7 text-slate-500">
                질문 검토는 별도 메뉴가 아니라 테스트셋 생성 흐름에 포함됩니다. 문구를 수정하면 다운로드 상태가 다시 대기로 바뀝니다.
              </p>
            </div>
          ...
          <div className="divide-y divide-slate-100">
            {generatedQuestions.map((question, index) => (
              <div key={question.id} className="grid gap-5 px-7 py-5 lg:grid-cols-[112px_minmax(0,1fr)]">
                <div>
          ...
          <div className="grid gap-4 border-t border-slate-100 bg-slate-50 px-7 py-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
            <div>
              ...
              <p className="mt-1 text-[15px] leading-7 text-slate-500">
                질문 CSV를 내려받은 뒤, 각 질문을 사용자의 RAG 시스템에 입력해 question, answer 중심의 결과 파일을 준비하세요.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <button
        ...
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_440px]">
          <div className="rounded-2xl border border-slate-200 bg-white p-7 shadow-[0_8px_28px_rgba(15,23,42,0.04)]">
            ...
            <h3 className="mt-2 text-xl font-semibold text-slate-950">질문 세트가 아직 없습니다</h3>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-7 shadow-[0_8px_28px_rgba(15,23,42,0.04)]">
            ...
            <h3 className="mt-2 text-xl font-semibold text-slate-950">결과 CSV는 이렇게 준비합니다</h3>
            <p className="mt-2 text-[15px] leading-7 text-slate-500">
...
문서의 핵심 목적은 무엇인가요?,문서 기반으로 질문을 만들고 답변 품질을 평가합니다.`}</pre>
          </div>
        </section>
      )}
    </div>
